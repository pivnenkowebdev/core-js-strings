...
function getStringLength(value) {
  ...
  return lengthStr > 0 ? lengthStr : 0;
}
...
function reverseWords(str) {
  const arrFromOrig = str.split(' ');
  const reverseStr = arrFromOrig.map((word) => {
...
function invertCase(str) {
  const newArr = str.split('').map((char) => {
    ...
    if (char === char.toLowerCase()) {
      changeSymbol = char.toUpperCase();
    } else if (char === char.toUpperCase()) {
      changeSymbol = char.toLowerCase();
    }
    return changeSymbol;
  });
  return newArr.join('');
...
function encodeToRot13(str) {
  ...
  const result = str.split('').map((char) => {
    let newChar;
    const positionInAlphabet = alphabet.indexOf(char.toLowerCase());
    if (positionInAlphabet !== -1) {
      newChar = alphabet[positionInAlphabet + 13];
