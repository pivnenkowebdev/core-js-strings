...
function removeFirstOccurrences(str, value) {
  ...
  const endIndex = firstIndex + value.length;
  const firstPart = str.slice(0, firstIndex);
  const secondPart = str.slice(endIndex);
...
function reverseWords(str) {
  ...
  const reverseWord = splitStr.map((word) => {
...
function invertCase(str) {
  ...
  const newLetters = arrLetters.map((letter) => {
...
function encodeToRot13(str) {
  ...
  const result = str.replace(
    /[a-z]/gi,
    (symb) => alphabet[alphabet.indexOf(symb) + 13]
  );
  return result;
}
